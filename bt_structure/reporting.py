...
class ShapReport:
    ...
    def generate_shap_explainability(self, strategy_name, asset_returns):
        ...
        shap_values = explainer(self.asset_returns)
        shap.summary_plot(shap_values, self.asset_returns)


class DashReport:
    def __init__(self, asset_returns, strategy_results, benchmark_returns):
    ...
    def _generate_performance_report(self):
        ...
        for name, results in self.strategy_results.items():
            self.rolling_metrics = {
                ...
                'weights': results['weights'], 'positions': results['positions'],
    ...
    def _setup_layout(self):
        self.app.layout = html.Div([
            ...
            *[dcc.Graph(id=f'asset-{metric}-graph') for metric in
              ['weights', 'positions', 'asset_pnl', 'asset_cumulative_returns']],
    ...
    def _setup_callbacks(self):
        @self.app.callback(
            [Output(f'asset-{metric}-graph', 'figure') for metric in ['weights', 'positions', 'asset_pnl', 'asset_cumulative_returns']],
        ...
        def update_asset_graphs(selected_strategy):
            figures = [
                ...
                if metric_name in ['weights', 'positions', 'asset_pnl', 'asset_cumulative_returns']
            ]
